...
function save_mempool_tx(txid, cb) {
...
function update_mempool_tx(txid, cb) {
	lib.get_rawtransaction(txid, function(tx) {
		if (tx != 'There was an error. Check your console.') {
			if (tx.blockhash) {
...
module.exports = {
  ...
  connect: function(database, cb) {
    mongoose.connect(database, {useMongoClient: true}).then(function(err) {
      return cb();
    }).catch(function(err) {
		console.log(err);
  ...
  get_addresses: function(hashes, includeTxs, cb) {
	Address.aggregate([
		{$match: {a_id: {$in: hashes}}},
		{
			$project: {
				address: "$a_id",
				balance: 1,
		...
		if (addrs) {
			if (includeTxs) {
				lib.get_blockcount(function(number) {
					lib.syncLoop(addrs.length, function(loop) {
						var i = loop.iteration(),
							txs = addrs[i].txs,
							txIds = [];

						lib.syncLoop(txs.length, function(sub) {
							var j = sub.iteration();
							txIds.push(txs[j].addresses);
							sub.next();

						}, function() {
							module.exports.get_txs_by_ids(txIds, function(newTxs) {
								addrs[i].blockcount = number;
								addrs[i].txs = newTxs;
								loop.next();
							});
						});
  ...
  get_txs_by_ids: function(txids, cb) {
	  Tx.aggregate([
		  {$match: {txid: {$in: txids}}},
		  {$sort: {timestamp: -1, blockindex: -1, blocktime: -1}}
  ...
  // MEMPOOL
  update_mempool_db: function(timeout, cb) {
	  lib.clearCache();
	  lib.get_rawmempool(function(tx_list) {
		  lib.syncLoop(tx_list.length, function(subloop) {
			  var i = subloop.iteration();
			  Tx.findOne({txid: tx_list[i]}, function(err, tx) {
				  if (tx) {
					  tx = null;
					  subloop.next();
				  }
				  else {
						save_mempool_tx(tx_list[i], function(err) {
							if (err) {
								console.log(err);
							}
							else {
								console.log('%s: %s', -1, tx_list[i]);
							}
							setTimeout(function() {
								tx = null;
								subloop.next();
  ...
  // MEMPOOL to Blocks
  update_mined_blocks_db: function(timeout, cb) {
	Tx.find({blockindex:-1}).exec(function(err, txs){
		lib.syncLoop(txs.length, function(loop) {
			var i = loop.iteration();
			update_mempool_tx(txs[i].txid, function(err, height) {
				if (err) {
					console.log(err);
				}
				else if (height) {
					console.log('Mined %s: %s', height, txs[i].txid);
				}
  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    ...
    lib.syncLoop((end - start) + 1, function (loop) {
      ...
      if (x % settings.update_stat_blocks === 0) {

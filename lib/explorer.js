...
var cacher = (function() {
	...
	function write(type, id, value) {
//		cacheData[type + '___' + id] = value;

		if (!cacheData[type]) {
			cacheData[type] = {};
		}
		
		cacheData[type][id] = value;
	}
	
	function read(type, id) {
//		if (cacheData[type + '___' + id]) {
//			return cacheData[type + '___' + id];
		if (cacheData[type] && cacheData[type][id]) {
			return cacheData[type][id];
		}
		else {
			return null;
		}
...
var BClient = (function() {
	var Client = new Bitcoin({
		host: settings.wallet.host,
		port: settings.wallet.port,
		username: settings.wallet.user,
		password: settings.wallet.pass
	});
	...
	function call(method, params, cb) {
		...
		Client.command(cmd).then(function(response){
			...
			console.log(reject);
			cb("There was an error. Check your console.", null);
		});
	}
...
module.exports = {
  ...
  get_blockhash: function(height, cb, http) {
		var data = cacher.read('blockhash', height);
	  
		if (!data) {
			cacher.clear('blockhash');
			
			if (http) {
				var uri = base_url + 'getblockhash?height=' + height;
				request({uri: uri, json: true}, function (error, response, body) {
					cacher.write('blockhash', height, body);
					return cb(body);
				});
			}
			else {
				BClient.call('getblockhash', height, function(err, resp) {
					if (err) {
						console.log(err);
						return cb('There was an error. Check your console.');
					}
					else {
						cacher.write('blockhash', height, resp);
						return cb(resp);
					}
				});
			}
  ...
  get_block: function(hash, cb, http) {
	  var data = cacher.read('block', hash);
	  ...
	  if (!data) {
			...
			if (http) {
				...
				request({uri: uri, json: true}, function (error, response, body) {
					cacher.write('block', hash, body);
					return cb(body);
				});
			}
			else {
				BClient.call('getblock', hash, function(err, resp) {
					if (err) {
						console.log(err);
						return cb('There was an error. Check your console.');
					}
					else {
						cacher.write('block', hash, resp);
						return cb(resp);
					}
				});
			}
  ...
  get_rawtransaction: function(hash, cb, http) {
		if (http) {
			...
			BClient.call('getrawtransaction', [hash, 1], function(err, resp) {
				if (err) {
					console.log(err);
					return cb('There was an error. Check your console.');
				}
 ...
 * Cached Tx Data
 *	get_rawtransaction: function(hash, cb, http) {
		var data = cacher.read('rawtransaction', hash);
	  
		if (!data) {
			if (http) {
				var uri = base_url + 'getrawtransaction?txid=' + hash + '&decrypt=1';
				request({uri: uri, json: true}, function (error, response, body) {
					cacher.write('rawtransaction', hash, body);
					return cb(body);
				});
			}
			else {
				BClient.call('getrawtransaction', [hash, 1], function(err, resp) {
					if (err) {
						...
						cacher.write('rawtransaction', hash, resp);
						return cb(resp);
					}
				});
			}
  ...
  get_rawmempool: function(cb, http) {
	...
	if (http) {
		...
		BClient.call('getrawmempool', null, function(err, resp) {
			if (err) {
				console.log(err);
				return cb('There was an error. Check your console.');
			}
...
//	console.log('GB: ' + process.memoryUsage().heapTotal / 1024 / 1024 / 1024);
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout && vout.length && vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') { 
      ...
      if (vout && vout.length && vout[0].scriptPubKey.type == 'nonstandard') {
